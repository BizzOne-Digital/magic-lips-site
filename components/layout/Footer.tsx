...
export default function Footer() {
  ...
  const { content } = usePageContent<{ footerTagline: string }>(
    "home",
    PAGE_CONTENT_DEFAULTS.home as unknown as { footerTagline: string }
  );
  ...
  const ig = settings.instagramHandle?.replace("@", "") || "magiclips2013";
  const tt = settings.tiktokHandle?.replace("@", "") || "magiclips02";
    ...
    <footer className="bg-[#F0ECFB] border-t border-[#9D8EC4]/10 text-[#1F2937]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          <div>
            ...
            <p className="text-sm text-gray-600 leading-relaxed">
              {content.footerTagline || "Your premier destination for premium lip beauty."}
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gray-600">
              <li>
                <a href={`mailto:${settings.email}`} className="flex items-center gap-2 hover:text-[#9D8EC4] transition-colors duration-200">
                  <Mail className="w-4 h-4 flex-shrink-0" />
                  {settings.email}
                </a>
              </li>
              <li>
                <a href={phoneHref} className="flex items-center gap-2 hover:text-[#9D8EC4] transition-colors duration-200">
        ...
        <div className="mt-10 pt-6 border-t border-[#9D8EC4]/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-500">
          <p>{settings.footerCopyright || `© ${new Date().getFullYear()} Magic Lips. All rights reserved.`}</p>
